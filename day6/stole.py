class Guard:
    ...
    def find_next_move(self):
        self.possible_move = [
            self.position[x] + self.dir_vector[self.dir][x] for x in range(2)
        ]
    ...
    def find_potential_loops(self):
        with open("loop_points.txt", "w") as f:
            for obstical_potential in self.guard.main_path:
                old_part2 = self.part2  # Remember old value
                self.room_map[obstical_potential] = "#"
                self.guard.restart_loop_tracker()
                self._reset_map()
                self.get_patrol_route(part_1=False)
                if self.part2 > old_part2:  # If it increased
                    f.write(f"{obstical_potential[0]},{obstical_potential[1]}\n")
                self.room_map[obstical_potential] = "."
